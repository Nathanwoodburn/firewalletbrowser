...
def index():
    ...
    for function in dashFunctions:
        functionOutput = plugins_module.runPluginFunction(
            function["plugin"], function["function"], {}, request.cookies.get("account")
        )
        plugins += render.plugin_output_dash(
            functionOutput,
            plugins_module.getPluginFunctionReturns(
                function["plugin"], function["function"]
            ),
        )
    ...
    for alert in alerts:
        ...
        plugins += render_template(
            "components/dashboard-alert.html", name=alert["name"], output=output_html
        )
...
def transactions():
    ...
    return render_template("tx.html", account=account, page=page, txCount=0)
...
def send():
    ...
    if address is None or amount is None:
        return redirect(
            f"/send?message=Invalid address or amount&address={address}&amount={amount}"
        )

    address_check = account_module.check_address(address.strip(), True, True)
    if not address_check:
        return redirect(
            f"/send?message=Invalid address&address={address}&amount={amount}"
        )
    ...
    if re.match(r"^\d+(\.\d+)?$", amount) is None:
        return redirect(
            f"/send?message=Invalid amount&address={address}&amount={amount}"
        )
    ...
    if amount > account_module.getBalance(account)["available"] - fees:
        return redirect(
            f"/send?message=Not enough funds to transfer&address={address}&amount={amount}"
        )
    ...
    content += (
        f"This will cost {amount} HNS + mining fees and is not able to be undone."
    )
    ...
    if "error" in response and response["error"] is not None:
        ...
        if isinstance(response["error"], dict):
            if "message" in response["error"]:
                return redirect(
                    f"/send?message={response['error']['message']}&address={address}&amount={amount}"
                )
            else:
                return redirect(
                    f"/send?message={response['error']}&address={address}&amount={amount}"
                )
...
def search():
    ...
    for function in searchFunctions:
        functionOutput = plugins_module.runPluginFunction(
            function["plugin"], function["function"], {"domain": search_term}, account
        )
        plugins += render.plugin_output(
            functionOutput,
            plugins_module.getPluginFunctionReturns(
                function["plugin"], function["function"]
            ),
    ...
    if domain_info:
        ...
        if "info" in domain_info and "owner" in domain_info["info"]:
            owner = account_module.getAddressFromCoin(
                domain_info["info"]["owner"]["hash"],
                domain_info["info"]["owner"]["index"],
            )
...
def manage(domain: str):
    ...
    if (
        domain_info["info"] is not None
        and "stats" in domain_info["info"]
        and "daysUntilExpire" in domain_info["info"]["stats"]
    ):
    ...
    for function in domainFunctions:
        functionOutput = plugins_module.runPluginFunction(
            function["plugin"], function["function"], {"domain": domain}, account
        )
        plugins += render.plugin_output(
            functionOutput,
            plugins_module.getPluginFunctionReturns(
...
def cancelTransfer(domain: str):
    ...
    if "error" in response:
        if response["error"] is not None:
            ...
            return redirect(
                "/manage/" + domain + "?error=" + response["error"]["message"]
            )
...
def revokeConfirm(domain: str):
    ...
    if check != str(revokeCheck):
        return redirect(
            "/manage/" + domain + "?error=An error occurred. Please try again."
        )
        ...
        if dnsType != "DS":
            ...
            if len(ds) != 4:
                ...
                return redirect(
                    ...
                    + domain
                    + "/edit?dns="
                    + urllib.parse.quote(str(raw_dns))
                    + "&error=Invalid DS record"
                )
...
def editSave(domain: str):
    ...
    if "error" in response:
        ...
        return redirect(
            f"/manage/{domain}/edit?dns={raw_dns}&error={response['error']}"
        )
    return redirect(f"/success?tx={response['hash']}")
...
def signMessage(domain):
    ...
    signedMessage = account_module.signMessage(
        request.cookies.get("account"), domain, message
    )
    ...
    data = {"domain": domain, "message": message, "signature": signedMessage["result"]}

    content += (
        "Full information:<br><code style='text-align:left;display: block;'>"
        + json.dumps(data, indent=4).replace("\n", "<br>")
        + "</code><br><br>"
    )

    content += (
        "<textarea style='display: none;' id='data' rows='4' cols='50'>"
        + json.dumps(data)
        + "</textarea>"
    )
    ...
    content += (
        "<button id='copyButton' onclick='copyToClipboard()' class='btn btn-secondary'>Copy to clipboard</button>"
        + copyScript
    )

    return render_template(
        "message.html", account=account, title="Sign Message", content=content
    )
    ...
    if state == "CLOSED":
        if not domainInfo["info"]["registered"]:
            if account_module.isOwnDomain(account, domain):
                ...
                next_action = (
                    f'<a href="/auction/{domain}/register">Register Domain</a>'
                )
...
def bid_confirm(domain):
    ...
    # Send the bid
    response = account_module.bid(
        request.cookies.get("account"), domain, float(bid), float(blind)
    )
...
def open_auction(domain):
    ...
    if "error" in response:
        if response["error"] is not None:
            return redirect(
                "/auction/" + domain + "?error=" + response["error"]["message"]
            )
...
def registerdomain(domain):
    ...
    if "error" in response:
        return redirect(
            "/auction/" + domain + "?message=" + response["error"]["message"]
        )
    return redirect(f"/success?tx={response['hash']}")
...
def settings_action(action):
    ...
    if action == "api-info":
        ...
        return render_template(
            "message.html", account=account, title="API Information", content=content
        )
        ...
        try:
            with open(log_file, "rb") as f:
                response = requests.put(
                    f"https://upload.woodburn.au/{os.path.basename(log_file)}",
                    data=f,
                    headers={"Max-Days": "5"},
                )
            if response.status_code == 200 or response.status_code == 201:
                ...
                logger.error(
                    f"Failed to upload log: {response.status_code} {response.text}"
                )
                return redirect(
                    f"/settings?error=Failed to upload log: {response.status_code}"
                )
...
def upload_image():
    ...
    if file and file.filename:
        filepath = os.path.join(
            f"user_data/images/{account}.{file.filename.split('.')[-1]}"
        )
        file.save(filepath)
...
def latestVersion(branch):
    result = requests.get(
        "https://git.woodburn.au/api/v1/repos/nathanwoodburn/firewalletbrowser/branches"
    )
...
def login():
    ...
    if "message" in request.args:
        return render_template(
            "login.html", error=request.args.get("message"), wallets=wallets
        )

    return render_template("login.html", wallets=wallets)
...
def login_post():
    ...
    if account is None or password is None:
        ...
        wallets = render.wallets(wallets)
        return render_template(
            "login.html", error="Invalid account or password", wallets=wallets
        )
    ...
    if account.count(":") > 0:
        wallets = account_module.listWallets()
        wallets = render.wallets(wallets)
        return render_template("login.html", error="Invalid account", wallets=wallets)
...
def import_wallet():
    ...
    add_alert(
        "Rescan needed",
        "Please rescan the wallet after importing to see all transactions",
        account,
    )
    # Set the cookie
    response = make_response(redirect("/"))
...
def plugins_index():
    ...
    return render_template("plugins.html", account=account, plugins=plugins)
...
def plugin(ptype, plugin):
    ...
    if not data["verified"]:
        functions = (
            "<div class='container-fluid'><div class='alert alert-warning' role='alert'>This plugin is not verified and is disabled for your protection. Please check the code before marking the plugin as verified <a href='/plugin/"
            + plugin
            + "/verify' class='btn btn-danger'>Verify</a></div></div>"
            + functions
        )
    ...
    if function in plugins_module.getPluginFunctions(plugin):
        ...
        for input in inputs:
            ...
            if inputs[input]["type"] == "address":
                # Handle hip2
                address_check = account_module.check_address(
                    request_data[input], True, True
                )
                if not address_check:
        ...
        response = plugins_module.runPluginFunction(
            plugin, function, request_data, request.cookies.get("account")
        )
        if not response:
        ...
        for output in outputs:
            if outputs[output]["type"] == "file":
                ...
                return send_file(
                    data, as_attachment=True, download_name=outputs[output]["name"]
                )
...
def api_hsd(function):
    ...
    if account is None and request.authorization is not None:
        account = account_module.check_account(
            f"{request.authorization.username}:{request.authorization.password}"
        )
    ...
    if (
        function == "mempoolBids"
    ):  # This is a heavy function so only allow for logged in users
    ...
    if function == "nextAuctionState":
        ...
        if state == "CLOSED":
            ...
            if stats["blocksUntilReveal"] == 1:
                ...
                next += (
                    f"<br>Last chance to bid in {stats['blocksUntilReveal'] - 2} blocks"
                )
        ...
        return jsonify({"state": state, "next": next, "next_action": next_action})
